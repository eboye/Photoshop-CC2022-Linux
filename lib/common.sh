...
# xdotool is only needed when applying appearance configuration after install
check_xdotool() {
  if ! command -v xdotool >/dev/null 2>&1; then
    log_error "xdotool is required for appearance configuration"
    ...
    log_info "Re-run with --skip-appearance to bypass."
    exit 1
  fi
}
...
# Emit base Wine env exports as shell text, so install-time and runtime
# launcher share a single definition and cannot drift apart.
_wine_env_exports() {
  local wine_dir="$1"
  local wineprefix="$2"
  cat <<EOF
...
# Setup Wine environment in the current shell
setup_wine_env() {
  eval "$(_wine_env_exports "$1" "$2")"
...
# Write a standalone, sourceable env file the launcher can use
write_wine_env_file() {
  local file="$1"
  local wine_dir="$2"
  local wineprefix="$3"
  {
    ...
    echo "# Auto-generated by installer — do not edit by hand"
    _wine_env_exports "$wine_dir" "$wineprefix"
  } > "$file"
  chmod 644 "$file"
}
...
apply_appearance_config() {
  ...
  local theme_reg font_reg
  theme_reg="$(mktemp --suffix=.reg)"
  font_reg="$(mktemp --suffix=.reg)"
  trap 'rm -f "$theme_reg" "$font_reg"' RETURN
...
EOF
  ...
  "$wine_dir/bin/wine" regedit /S "$theme_reg" 2>/dev/null || \
    log_warning "Failed to apply theme registry"
...
EOF
  ...
  "$wine_dir/bin/wine" regedit /S "$font_reg" 2>/dev/null || \
    log_warning "Failed to apply font registry"
