...
# Anything left after '--' is the install dir
...
if [ "$DRY_RUN" = "true" ]; then
  cat <<EOF
[dry-run] Would install Photoshop 2021 with these settings:
...
cd "$INSTALL_DIR"
# Re-fetch if the local copy is missing, empty, or not executable
